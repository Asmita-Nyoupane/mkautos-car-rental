...
const ChatNow = () => {
    ...
    return (
        <div
            className={`fixed bottom-4 right-4 z-50 transform transition-all duration-700 ease-in-out ${visible ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'
                }`}
        >
            <Link href={'/'} className="flex items-center bg-black/50 py-2 px-4 rounded-full shadow-lg hover:bg-black/70 transition-colors duration-300 ease-in-out">
                ...
                />
                <span className="ml-2 text-white text-sm font-semibold hidden md:inline">Chat Now</span>
            </Link>
        </div>
    );
};
